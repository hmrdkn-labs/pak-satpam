...
export class GitHubSCMProvider implements SCMReadProvider {
  ...
  async getChangeEvidence(input: SCMChangeEvidenceInput): Promise<SCMChangeEvidenceResult> {
    ...
    if (prepared.input.pullRequest !== undefined) {
      return this.pullRequestEvidence(prepared);
    }
    const selector = prepared.input.commit ?? prepared.input.ref;
    if (selector === undefined) throw new SCMProviderError("malformed");
    const value = await this.json(`/repos/${prepared.input.repository}/commits/${encodeURIComponent(selector)}`, prepared.input.repository);
    ...
    const files = githubFiles(value.files);
    ...
    return finalizeSCMEvidence({
      ...
      files,
      providerTruncated: files.length >= prepared.budget.maxItems,
    });
  }
  ...
  private async pullRequestEvidence(prepared: ReturnType<typeof prepareSCMInput>): Promise<SCMChangeEvidenceResult> {
    ...
    const raw = await this.json(`/repos/${repositoryName}/pulls/${encodeURIComponent(pullRequestId)}`, repositoryName);
    ...
    const response = await this.request(`/repos/${repositoryName}/pulls/${encodeURIComponent(pullRequestId)}/files?per_page=${prepared.budget.maxItems}`, repositoryName);
    ...
    return finalizeSCMEvidence({
      ...
      pullRequest,
      files,
      providerTruncated: value.length >= prepared.budget.maxItems,
    });
  }

  private async json(path: string, repositoryName: string): Promise<Record<string, unknown>> {
    const response = await this.request(path, repositoryName);
    return boundedJson(response);
  }

  private async request(path: string, repositoryName: string): Promise<Response> {
    let token: string;
    ...
    try {
      return await this.#fetch(resolveSCMUrl(this.#apiBaseUrl, path), {
        method: "GET",
...
function githubFiles(value: unknown): RawSCMFile[] {
  if (!Array.isArray(value)) throw new SCMProviderError("malformed");
  return value.map((item): RawSCMFile => {
